...
def sliding_window_batch_inference(model, tokenizer, input_ids, max_length=510, overlap=150):
    batch_predictions = []
    ...
    for i in range(len(input_ids)):
        ...
        if token_len <= max_length:
            with torch.no_grad():
                logit = model(input_ids[i][0:510].unsqueeze(0))
...
def test(args, model, tokenizer):
    ...
    logger.info("  Batch size = %d", args.eval_batch_size)
    model.eval()
    overlap1=[0,50,100,150,200,250,300,350,400,450]
    for o in overlap1:
        labels = []
        ...
        for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
            inputs = batch[0].to(args.device)
            label = batch[1].to(args.device)
            batch_predictions = sliding_window_batch_inference(model, tokenizer, inputs,overlap=o)
            for k in range(20):
